...
def aug_depth_params(K, n_steps=75):
    ...
    # roll augmentations
    roll_aug = (
        [i for i in range(0, n_steps + 1, 2)]
        + [i for i in range(n_steps, -n_steps - 1, -2)]
        + [i for i in range(-n_steps, 1, 2)]
    )
    ang_y, ang_z = 0.0, 0.0
    for angle in roll_aug:
        ang_x = _DEGTORAD * (angle / n_steps * 10.0)
        aug_params.append([torch.inverse(K), ang_x, ang_y, ang_z])

    # pitch augmentations
    pitch_aug = (
        [i for i in range(0, 50 + 1, 2)]
        + [i for i in range(50, -50 - 1, -2)]
        + [i for i in range(-50, 1, 2)]
    )
    ang_x, ang_z = 0.0, 0.0
    for angle in pitch_aug:
        ang_y = _DEGTORAD * (angle / 10.0)
        aug_params.append([torch.inverse(K), ang_x, ang_y, ang_z])
    ...
    for f_idx in range(100 + 1):
        f_scale = (f_idx / 100.0 * focal_ratio_aug + (1 - f_idx / 100.0))[:, None]
        K_aug = K.clone()
        K_aug[:, :, 0, 0] *= f_scale
        K_aug[:, :, 1, 1] *= f_scale
    ...
    for f_idx in range(50 + 1):
        f_scale = (f_idx / 50.0 * focal_ratio + (1 - f_idx / 50.0) * focal_ratio_aug)[
            :, None
        ]
        K_aug = K.clone()
        K_aug[:, :, 0, 0] *= f_scale
