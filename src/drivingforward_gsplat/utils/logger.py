...
def plot_norm_tb(writer, step, img, title, j=0):
    ...
    """
    img_resized = torch.clamp(resize_for_tb(img), 0.0, 1.0)
    writer.add_image(title, img_resized[j].data, step)
...
class Logger:
    ...
    def log_time(self, epoch, batch_idx, duration, loss, start_time):
        ...
        print("")
        print(
            f"epoch: {epoch:2d} | batch: {batch_idx:6d} |"
    ...
    def log_tb(self, mode, inputs, outputs, losses, step):
        ...
        for cam_id in range(self.num_cams):
            ...
            plot_tb(
                writer,
                step,
                inputs[("color", 0, scale)][:, cam_id, ...],
                set_tb_title("cam", cam_id, "frame_0"),
            )  # frame_id 0
            plot_tb(
                writer,
                step,
                inputs[("color", 1, scale)][:, cam_id, ...],
                set_tb_title("cam", cam_id, "frame_1"),
            )  # frame_id 1
            plot_tb(
                writer,
                step,
                inputs[("color", -1, scale)][:, cam_id, ...],
                set_tb_title("cam", cam_id, "frame_-1"),
            )  # frame_id -1
            plot_disp_tb(
                writer,
                step,
                target_view[("disp", scale)],
                set_tb_title("cam", cam_id, "disp"),
            )  # disparity
            ...
            plot_tb(
                writer,
                step,
                target_view[("reproj_loss", scale)],
                set_tb_title("cam", cam_id, "reproj"),
            )  # reprojection image
            plot_tb(
                writer,
                step,
                target_view[("reproj_mask", scale)],
                set_tb_title("cam", cam_id, "reproj_mask"),
            )  # reprojection mask
            plot_tb(
                writer,
                step,
                inputs["mask"][:, cam_id, ...],
                set_tb_title("cam", cam_id, "self_occ_mask"),
            )
            ...
            if self.spatio:
                plot_norm_tb(
                    writer,
                    step,
                    target_view[("overlap", 0, scale)],
                    set_tb_title("cam", cam_id, "sp"),
                )
                plot_tb(
                    writer,
                    step,
                    target_view[("overlap_mask", 0, scale)],
                    set_tb_title("cam", cam_id, "sp_mask"),
                )
            ...
            if self.spatio_temporal:
                for frame_id in self.frame_ids:
                    ...
                    plot_norm_tb(
                        writer,
                        step,
                        target_view[("color", frame_id, scale)],
                        set_tb_title("cam", cam_id, "pred_", frame_id),
                    )
                    plot_norm_tb(
                        writer,
                        step,
                        target_view[("overlap", frame_id, scale)],
                        set_tb_title("cam", cam_id, "sp_tm_", frame_id),
                    )
                    plot_tb(
                        writer,
                        step,
                        target_view[("overlap_mask", frame_id, scale)],
                        set_tb_title("cam", cam_id, "sp_tm_mask_", frame_id),
                    )
            ...
            if self.gaussian:
                if self.novel_view_mode == "SF":
                    ...
                    raise ValueError(
                        f"Novel view mode {self.novel_view_mode} not supported."
                    )
                plot_norm_tb(
                    writer,
                    step,
                    target_view[("gaussian_color", frame_id, scale)],
                    set_tb_title("cam", cam_id, f"gaussian_pred_frame_{frame_id}"),
                )
    ...
    def log_result(self, inputs, outputs, idx, syn_visualize=False):
        ...
        for cam_id in range(self.num_cams):
            ...
            for jdx, disp in enumerate(disps):
                ...
                disp.save(
                    os.path.join(self.cam_paths[cam_id], f"{cur_idx:03d}_disp.jpg")
                )
    ...
    def compute_depth_losses(self, inputs, outputs, vis_scale=False):
        ...
        for cam in range(self.num_cams):
            ...
            depth_pred = torch.clamp(
                F.interpolate(depth_pred, [h, w], mode="bilinear", align_corners=False),
                min_eval_depth,
                max_eval_depth,
            )
            ...
            mask = (
                (depth_gt > min_eval_depth)
                * (depth_gt < max_eval_depth)
                * inputs["mask"][:, cam, ...]
            )
            mask = mask.bool()
            ...
            depth_pred_metric = torch.clamp(
                depth_pred, min=min_eval_depth, max=max_eval_depth
            )
            depth_errors_metric = cal_depth_error(depth_pred_metric, depth_gt)

            depth_pred_median = torch.clamp(
                depth_pred * scale_val, min=min_eval_depth, max=max_eval_depth
            )
        ...
        for key in error_metric_dict.keys():
            error_metric_dict[key] = (
                error_metric_dict[key].cpu().numpy() / self.num_cams
            )
            error_median_dict[key] = (
                error_median_dict[key].cpu().numpy() / self.num_cams
            )
