...
from drivingforward_gsplat.utils import Logger
...
class DrivingForwardTrainer:
    ...
    def train(self, model, data_loader, start_time):
        ...
        pbar = tqdm(
            ...
            desc="training on epoch {}".format(self.epoch),
            mininterval=100,
        )
    ...
    def compute_reconstruction_metrics(self, inputs, outputs):
        ...
        for cam in range(self.num_cams):
            ...
            if self.save_images:
                ...
                if self.novel_view_mode == "SF":
                    self.save_image(
                        image, Path(self.save_path) / inputs["token"][0] / f"{cam}.png"
                    )
                    self.save_image(
                        rgb_gt,
                        Path(self.save_path) / inputs["token"][0] / f"{cam}_gt.png",
                    )
                    self.save_image(
                        inputs[("color", 0, 0)][:, cam, ...],
                        Path(self.save_path) / inputs["token"][0] / f"{cam}_0_gt.png",
                    )
                elif self.novel_view_mode == "MF":
                    self.save_image(
                        image, Path(self.save_path) / inputs["token"][0] / f"{cam}.png"
