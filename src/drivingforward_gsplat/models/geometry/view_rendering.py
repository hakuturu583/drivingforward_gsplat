...
class ViewRendering(nn.Module):
    ...
    def init_project_imgs(self, rank):
        project_imgs = {}
        project_imgs = Projection(self.batch_size, self.height, self.width, rank)
        return project_imgs
    ...
    def get_mean_std(self, feature, mask):
        ...
        """
        _, c, h, w = mask.size()
        mean = (feature * mask).sum(dim=(1, 2, 3), keepdim=True) / (
    ...
    def get_virtual_image(
        self, src_img, src_mask, tar_depth, tar_invK, src_K, T, scale=0
    ):
        """
        ...
        invalid_mask = (
            torch.logical_or(pix_coords > 1, pix_coords < -1).sum(dim=1, keepdim=True)
            > 0
        )
        return img_warped, (~invalid_mask).float() * mask_warped

    def get_virtual_depth(
        self,
        src_depth,
        src_mask,
        src_invK,
        ...
        return (
            depth_warped,
            (~invalid_mask).float() * mask_warped * valid_depth_min * valid_depth_max,
        )
        ...
        for scale in self.scales:
            ...
            for frame_id in self.frame_ids[1:]:
                ...
                warped_img, warped_mask = self.get_virtual_image(
                    src_color, src_mask, ref_depth, ref_invK, ref_K, T, source_scale
                )

                if self.intensity_align:
                    warped_img = self.get_norm_image_single(
                        ref_color, ref_mask, warped_img, warped_mask
                    )
