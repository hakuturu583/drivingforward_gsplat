...
from drivingforward_gsplat.dataset import construct_dataset
...
class DrivingForwardModel(BaseModel):
    ...
    def set_train_dataloader(self, cfg, rank):
        ...
        self.num_total_steps = (
            num_train_samples // (self.batch_size * self.world_size) * self.num_epochs
        )
    ...
    def process_batch(self, inputs, rank):
        ...
        for key, ipt in inputs.items():
            if key not in _NO_DEVICE_KEYS:
                if "context" in key:
                    inputs[key] = [
                        ipt[k].float().to(rank) for k in range(len(inputs[key]))
                    ]
                if "ego_pose" in key:
                    inputs[key] = [
                        ipt[k].float().to(rank) for k in range(len(inputs[key]))
        ...
        for cam in range(self.num_cams):
            if self.mode != "train":
                outputs[("cam", cam)].update(
                    {("cam_T_cam", 0, 1): inputs[("cam_T_cam", 0, 1)][:, cam, ...]}
                )
                outputs[("cam", cam)].update(
                    {("cam_T_cam", 0, -1): inputs[("cam_T_cam", 0, -1)][:, cam, ...]}
                )
        ...
        for cam in range(self.num_cams):
            ...
            for scale in self.scales:
                ...
                if self.novel_view_mode == "MF":
                    ...
                    outputs[("cam", cam)][("depth", -1, scale)] = self.to_depth(
                        disp_last, ref_K
                    )
    ...
    def to_depth(self, disp_in, K_in):
        ...
        disp_in = F.interpolate(
            disp_in, [self.height, self.width], mode="bilinear", align_corners=False
        )
    ...
    def get_gaussian_data(self, inputs, outputs, cam):
        ...
        if self.novel_view_mode == "MF":
            for frame_id in self.frame_ids:
                if frame_id == 0:
                    outputs[("cam", cam)][("e2c_extr", frame_id, 0)] = inputs[
                        "extrinsics_inv"
                    ][:, cam, ...]
                    outputs[("cam", cam)][("c2e_extr", frame_id, 0)] = inputs[
                        "extrinsics"
                    ][:, cam, ...]
                    FovX_list = []
                    FovY_list = []
                    ...
                    for i in range(bs):
                        ...
                        world_view_transform_list.append(
                            world_view_transform.unsqueeze(0)
                        )
                        full_proj_transform_list.append(
                            full_proj_transform.unsqueeze(0)
                        )
                        camera_center_list.append(camera_center.unsqueeze(0))

                    outputs[("cam", cam)][("FovX", frame_id, 0)] = torch.tensor(
                        FovX_list
                    ).cuda()
                    outputs[("cam", cam)][("FovY", frame_id, 0)] = torch.tensor(
                        FovY_list
                    ).cuda()
                    outputs[("cam", cam)][
                        ("world_view_transform", frame_id, 0)
                    ] = torch.cat(world_view_transform_list, dim=0)
                    outputs[("cam", cam)][
                        ("full_proj_transform", frame_id, 0)
                    ] = torch.cat(full_proj_transform_list, dim=0)
                    outputs[("cam", cam)][("camera_center", frame_id, 0)] = torch.cat(
                        camera_center_list, dim=0
                    )
                else:
                ...
                rot_maps, scale_maps, opacity_maps, sh_maps = self.gs_net(
            ...
            frame_id = 0
            outputs[("cam", cam)][("e2c_extr", frame_id, 0)] = inputs["extrinsics_inv"][
                :, cam, ...
            ]
            outputs[("cam", cam)][("c2e_extr", frame_id, 0)] = inputs["extrinsics"][
                :, cam, ...
            ]
            outputs[("cam", cam)][("xyz", frame_id, 0)] = depth2pc(
                outputs[("cam", cam)][("depth", frame_id, 0)],
            ...
            for frame_id in self.frame_ids[1:]:
                ...
                for i in range(bs):
                    ...
                    world_view_transform = torch.matmul(
                        T_i, torch.tensor(extr).cuda()
                    ).transpose(0, 1)
                    # full_proj_transform: (E^T K^T) = (K E)^T
                    full_proj_transform = (
                        world_view_transform.unsqueeze(0).bmm(
                ...
                outputs[("cam", cam)][("full_proj_transform", frame_id, 0)] = torch.cat(
    ...
    def pred_gaussian_imgs(self, inputs, outputs, cam):
        if self.novel_view_mode == "MF":
            outputs[("cam", cam)][("gaussian_color", 0, 0)] = pts2render(
                inputs=inputs,
                outputs=outputs,
            ...
            for novel_frame_id in self.frame_ids[1:]:
                outputs[("cam", cam)][
                    ("gaussian_color", novel_frame_id, 0)
                ] = pts2render(
                    inputs=inputs,
                    outputs=outputs,
                    cam_num=self.num_cams,
