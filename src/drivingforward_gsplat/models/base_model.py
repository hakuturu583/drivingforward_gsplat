...
class BaseModel:
    ...
    def save_model(self, epoch):
        curr_model_weights_dir = os.path.join(
            self.save_weights_root, f"weights_{epoch}"
        )
    ...
    def load_weights(self):
        assert os.path.isdir(
            self.load_weights_dir
        ), f"\tCannot find {self.load_weights_dir}"
        ...
        for n in self.models_to_load:
            ...
            # load parameters
            pre_trained_dict = {
                k: v for k, v in pre_trained_dict.items() if k in model_dict
            }
        ...
        if self.mode == "train":
            # loading adam state
            optim_file_path = os.path.join(
                self.load_weights_dir, f"{_OPTIMIZER_NAME}.pth"
            )
            if os.path.isfile(optim_file_path):
                try:
                    ...
                    print(
                        f"\tCannnot load {_OPTIMIZER_NAME} - the optimizer will be randomly initialized"
                    )
            else:
                print(
                    f"\tCannot find {_OPTIMIZER_NAME} weights, so the optimizer will be randomly initialized"
                )
