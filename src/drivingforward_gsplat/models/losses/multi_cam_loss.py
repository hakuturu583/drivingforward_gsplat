...
class MultiCamLoss(SingleCamLoss):
    ...
    def compute_spatio_loss(
        self, inputs, target_view, cam=None, scale=None, ref_mask=None
    ):
        """
    ...
    def compute_spatio_tempo_loss(
        ...
        spatio_tempo_mask, _ = torch.max(
            spatio_tempo_masks.float(), dim=1, keepdim=True
        )
    ...
    def compute_pose_con_loss(
        ...
        for frame_id in self.frame_ids[1:]:
            ...
            t_diff = torch.norm(
                ref_T[:, :3, 3] - cur_T_aligned[:, :3, 3], p=2, dim=1
            ).mean()
    ...
    def forward(self, inputs, outputs, cam):
        loss_dict = {}
        cam_loss = 0.0  # loss across the multi-scale
        ...
        for scale in self.scales:
            ...
            spatio_tempo_loss = self.compute_spatio_tempo_loss(
                inputs, target_view, **kargs
            )

            if self.gaussian:
                gaussian_loss = self.compute_gaussian_loss(
                    inputs, target_view, cam, scale
                )
            ...
            cam_loss += (
                self.spatio_coeff * spatio_loss
                + self.spatio_tempo_coeff * spatio_tempo_loss
            )
            if self.gaussian:
