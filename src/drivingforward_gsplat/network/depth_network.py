...
class DepthNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.encoder = ResnetEncoder(
            self.num_layers, self.weights_init, 1
        )  # number of layers, pretrained, number of input images
        del self.encoder.encoder.fc  # del fc in weights_init
        enc_feat_dim = sum(self.encoder.num_ch_enc[self.fusion_level :])
        self.conv1x1 = conv2d(
            enc_feat_dim, self.fusion_feat_in_dim, kernel_size=1, padding_mode="reflect"
        )

        # fusion net
        fusion_feat_out_dim = self.encoder.num_ch_enc[self.fusion_level]
        self.fusion_net = VFNet(
            cfg, self.fusion_feat_in_dim, fusion_feat_out_dim, model="depth"
        )
    ...
    def forward(self, inputs):
        ...
        sf_images = torch.stack(
            [inputs[("color_aug", 0, 0)][:, cam, ...] for cam in range(self.num_cams)],
            1,
        )
        if self.novel_view_mode == "MF":
            sf_images_last = torch.stack(
                [
                    inputs[("color_aug", -1, 0)][:, cam, ...]
                    for cam in range(self.num_cams)
                ],
                1,
            )
            sf_images_next = torch.stack(
                [
                    inputs[("color_aug", 1, 0)][:, cam, ...]
                    for cam in range(self.num_cams)
                ],
        ...
        if self.novel_view_mode == "MF":
            packed_feats_agg_last = self.conv1x1(
                torch.cat(packed_feats_last_list, dim=1)
            )
            packed_feats_agg_next = self.conv1x1(
                torch.cat(packed_feats_next_list, dim=1)
            )

        feats_agg = unpack_cam_feat(packed_feats_agg, self.batch_size, self.num_cams)
        if self.novel_view_mode == "MF":
            feats_agg_last = unpack_cam_feat(
                packed_feats_agg_last, self.batch_size, self.num_cams
            )
            feats_agg_next = unpack_cam_feat(
                packed_feats_agg_next, self.batch_size, self.num_cams
            )
        ...
        if self.novel_view_mode == "MF":
            ...
            for i in range(len(feat_in_last)):
                img_feat_last.append(
                    unpack_cam_feat(feat_in_last[i], self.batch_size, self.num_cams)
                )

            feat_in_next = packed_feats_next[:lev] + [fusion_dict_next["proj_feat"]]
            img_feat_next = []
            for i in range(len(feat_in_next)):
                img_feat_next.append(
                    unpack_cam_feat(feat_in_next[i], self.batch_size, self.num_cams)
                )
        ...
        depth_outputs = unpack_cam_feat(
            packed_depth_outputs, self.batch_size, self.num_cams
        )
        if self.novel_view_mode == "MF":
            depth_outputs_last = unpack_cam_feat(
                packed_depth_outputs_last, self.batch_size, self.num_cams
            )
            depth_outputs_next = unpack_cam_feat(
                packed_depth_outputs_next, self.batch_size, self.num_cams
            )
        ...
        for cam in range(self.num_cams):
            ...
            if self.novel_view_mode == "MF":
                ...
                outputs[("cam", cam)][("disp", -1, 0)] = depth_outputs_last[
                    ("disp", 0)
                ][:, cam, ...]
                outputs[("cam", cam)][("disp", 1, 0)] = depth_outputs_next[("disp", 0)][
                    :, cam, ...
                ]
...
class DepthDecoder(nn.Module):
    def __init__(
        ...
        for i in range(self.level_in, -1, -1):
            num_ch_in = (
                self.num_ch_enc[-1] if i == self.level_in else self.num_ch_dec[i + 1]
            )
            num_ch_out = self.num_ch_dec[i]
            self.convs[("upconv", i, 0)] = conv2d(
                num_ch_in, num_ch_out, kernel_size=3, nonlin="ELU"
            )
            ...
            num_ch_out = self.num_ch_dec[i]
            self.convs[("upconv", i, 1)] = conv2d(
                num_ch_in, num_ch_out, kernel_size=3, nonlin="ELU"
            )

        for s in self.scales:
            self.convs[("dispconv", s)] = conv2d(
                self.num_ch_dec[s], self.num_output_channels, 3, nonlin=None
            )
