...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model="depth"):
        ...
        self.voxel_end_p = [
            self.voxel_str_p[i] + self.voxel_unit_size[i] * (self.voxel_size[i] - 1)
            for i in range(3)
        ]

        # define a voxel space, [1, 3, z, y, x], each voxel contains its 3D position
        voxel_grid = self.create_voxel_grid(
            self.voxel_str_p, self.voxel_end_p, self.voxel_size
        )
        ...
        self.pixel_grid = self.create_pixel_grid(
            self.batch_size, self.img_h, self.img_w
        )
        ...
        self.depth_grid = self.create_depth_grid(
            self.batch_size, self.num_pix, self.proj_d_bins, depth_bins
        )
        ...
        if model == "depth":
            ...
            self.conv_non_overlap = conv1d(
                self.v_dim_no[0], self.v_dim_no[1], kernel_size=1
            )
    ...
    def create_pixel_grid(self, batch_size, height, width):
        ...
        pix_coords = (
            torch.stack(grid_xy, axis=0).unsqueeze(0).view(1, 2, height * width)
        )
    ...
    def backproject_into_voxel(self, feats_agg, input_mask, intrinsics, extrinsics_inv):
        ...
        for cam in range(self.num_cams):
            ...
            mask_img = F.interpolate(
                mask_img, [h_dim, w_dim], mode="bilinear", align_corners=True
            )
            ...
            pix_coords = self.calculate_sample_pixel_coords(
                K_mat, v_pts_local, w_dim, h_dim
            )
            ...
            feat_warped = torch.cat(
                [
                    ...
                    v_pts_local[:, 2:3, :] / (self.voxel_size[0]),
                ],
                dim=1,
            )
            feat_warped = feat_warped * valid_mask.float()

            voxel_feat_list.append(
                feat_warped
            )  # [[batch_size, feat_dim, n_voxels], ...]
            voxel_mask_list.append(valid_mask)

        # compute overlap region
        voxel_mask_count = torch.sum(
            torch.cat(voxel_mask_list, dim=1), dim=1, keepdim=True
        )

        if self.model == "depth":
            # discriminatively process overlap and non_overlap regions using different MLPs
            voxel_non_overlap = self.preprocess_non_overlap(
                voxel_feat_list, voxel_mask_list, voxel_mask_count
            )
            voxel_overlap = self.preprocess_overlap(
                voxel_feat_list, voxel_mask_list, voxel_mask_count
            )
            voxel_feat = (
                voxel_non_overlap + voxel_overlap
            )  # [batch_size, feat_dim, n_voxels]

        elif self.model == "pose":
            voxel_feat = torch.sum(
                torch.stack(voxel_feat_list, dim=1), dim=1, keepdim=False
            )
    ...
    def calculate_sample_pixel_coords(self, K, v_pts, w_dim, h_dim):
        ...
        pix_coords = pix_coords.view(
            self.batch_size, 2, self.n_voxels, 1
        )  # [batch_size, 2, n_voxels] -> [batch_size, 2, n_voxels, 1]
        pix_coords = pix_coords.permute(
            0, 2, 3, 1
        )  # [batch_size, 2, n_voxels, 1] -> [batch_size, n_voxels, 1, 2]
    ...
    def calculate_valid_mask(self, mask_img, pix_coords, v_pts_local):
        ...
        mask_oob = ~(
            torch.logical_or(pix_coords_mask > 1, pix_coords_mask < -1).sum(
                dim=1, keepdim=True
            )
            > 0
        )
    ...
    def preprocess_non_overlap(
        self, voxel_feat_list, voxel_mask_list, voxel_mask_count
    ):
        """
    ...
    def project_voxel_into_image(self, voxel_feat, inv_K, extrinsics):
        ...
        for cam in range(self.num_cams):
            ...
            cam_points = self.depth_grid * cam_points.view(
                self.batch_size, 3, 1, self.num_pix
            )
            cam_points = torch.cat(
                [cam_points, self.pixel_ones], dim=1
            )  # [b, 4, n_depthbins, n_pixels]
            cam_points = cam_points.view(
                self.batch_size, 4, -1
            )  # [b, 4, n_depthbins * n_pixels]
            ...
            for i in range(3):
                ...
                grid[:, :, i] = (
                    grid[:, :, i] - self.voxel_str_p[i]
                ) / v_length * 2.0 - 1.0

            grid = grid.view(
                self.batch_size, self.proj_d_bins, self.img_h, self.img_w, 3
            )
            proj_feat = F.grid_sample(
                voxel_feat,
                grid,
                mode="bilinear",
                padding_mode="zeros",
            ...
            proj_feat = proj_feat.view(
                b, self.proj_d_bins * self.v_dim_o[-1], self.img_h, self.img_w
            )
    ...
    def forward(self, inputs, feats_agg):
        ...
        voxel_feat = self.backproject_into_voxel(
            feats_agg, mask, K, extrinsics_inv
        )  # [batch_size, feat_dim, n_voxels]
